 ...
 * Robust, fault-tolerant seeder that populates verified students,
 * discussions, confessions, polls, comments, votes, and birthdays across all Indian colleges.
 *
...
async function withRetry<T>(fn: () => Promise<T>, retries = 3, delayMs = 1000): Promise<T> {
  ...
  while (attempt < retries) {
    try {
      ...
      console.warn(`Transient DB issue, retrying (${attempt}/${retries})...`);
...
async function main() {
  ...
  const allInstitutions = await withRetry(() => db.select().from(institutions));
  ...
  // Process colleges in sequential batches with retry protection
  for (let cIdx = 0; cIdx < allInstitutions.length; cIdx++) {
    const inst = allInstitutions[cIdx];

    await withRetry(async () => {
      // Generate 2-4 students per college
      const studentCount = randomBetween(2, 4);
      ...
      for (let sIdx = 0; sIdx < studentCount; sIdx++) {
        ...
        const uniqueSuffix = randomBetween(10, 99999);
        ...
        const isDobPrivate = Math.random() < 0.15;
        ...
        newStudents.push({
          id: `seed_user_${inst.id.slice(0, 8)}_${sIdx}_${uniqueSuffix}`,
          userId: `seed_auth_${inst.id.slice(0, 8)}_${sIdx}_${uniqueSuffix}`,
          username,
          displayName,
      ...
      // Create 1-2 authentic posts for this college
      const postCount = randomBetween(1, 2);
      for (let pIdx = 0; pIdx < postCount; pIdx++) {
        ...
        const uniquePostSuffix = randomBetween(10, 99999);
        const postId = `seed_post_${inst.id.slice(0, 8)}_${pIdx}_${uniquePostSuffix}`;
        ...
        // Add 1-2 comments
        const commentCount = randomBetween(1, 2);
        ...
        // Seed 2-4 upvotes
        const voteCount = randomBetween(2, 4);
