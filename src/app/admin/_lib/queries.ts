...
export async function getDashboardStats(db: Db): Promise<DashboardStats> {
	try {
		...
		const [
			...
			db.select({ total: count() }).from(comments).catch(() => [{ total: 0 }]),
			db.select({ open: sql<number>`count(*) filter (where ${reports.status} = 'OPEN')::int` }).from(reports).catch(() => [{ open: 0 }]),
			db.select({ total: count() }).from(institutions).catch(() => [{ total: 0 }]),
			db.select({ total: count() }).from(anonIdentityVault).catch(() => [{ total: 0 }]),
		]);
...
export async function getAuditPage(db: Db, limit = 50, offset = 0): Promise<{ rows: FullAuditEntry[]; total: number }> {
...
export async function getTotalEngagement(db: Db): Promise<EngagementTotals> {
	const [row] = await db
		.select({ votes: sql<number>`coalesce(sum(${votes.value}), 0)::int` })
		.from(votes);
