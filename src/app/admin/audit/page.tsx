              ...
              {rows.map((row) => (
                <tr key={row.id} className="border-b border-border hover:bg-muted/50">
                  <td className="px-5 py-3">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded border ${actionTone(row.action)}`}
                    >
                      {row.action.replaceAll("_", " ")}
                    </span>
                    {row.reason && (
                      <p className="text-[10px] text-muted-foreground mt-1 line-clamp-1">{row.reason}</p>
                    )}
                  </td>
