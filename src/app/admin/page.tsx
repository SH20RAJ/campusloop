...
export default async function AdminDashboard() {
  ...
  const [stats, activity, topColleges, auditLog, engagement, recentPosts, recentUsers] = await Promise.all([
    ...
    db.query.userProfiles
      .findMany({ orderBy: [desc(userProfiles.createdAt)], limit: 5, with: { institution: true } })
      .catch(() => []),
  ]);
    ...
    <div className="mx-auto max-w-6xl space-y-8">
      ...
      <section className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          accent="primary"
          href="/admin/users"
          hint={`${stats.activeUsers} active · ${stats.bannedUsers} banned`}
        />
        <StatCard
          label="Total Posts"
        ...
        <StatCard
          ...
          icon={EyeOff}
          accent={stats.pendingPosts > 0 ? "red" : "green"}
          href="/admin/review"
          hint="auto-flagged by safety engine"
        />
        <StatCard
          label="Open Reports"
          value={stats.openReports}
          icon={ShieldAlert}
          accent={stats.openReports > 0 ? "orange" : "green"}
          href="/admin/reports"
        />
        <StatCard
          label="Comments"
        ...
        <StatCard
          ...
          value={stats.anonymousPosts}
          icon={Ghost}
          accent="primary"
          hint={`${stats.anonVaultEntries} sealed identities`}
        />
        <StatCard
          label="Colleges"
        ...
        />
        <StatCard
          label="LP Circulating"
          value={stats.lpCirculating.toLocaleString()}
          icon={Zap}
          accent="violet"
          hint={`${engagement.votes.toLocaleString()} total votes`}
        />
      </section>
          ...
          <div className="divide-y divide-border">
            {recentPosts.map((post) => (
              <Link
                ...
                className="block py-3 first:pt-0 last:pb-0 space-y-1 hover:bg-muted/40 rounded-lg px-2 -mx-2 transition-colors"
              >
      ...
      <section className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-2">
            {topColleges.map((c, i) => (
              ...
              >
                ...
                <span className="text-xs font-semibold text-foreground truncate flex-1 group-hover:text-primary transition-colors">
                  {c.name.split(",")[0]}
                </span>
                <span className="text-[10px] text-muted-foreground shrink-0">
                  {c.postCount} posts · {c.students} students
                </span>
              </Link>
            ))}
            {topColleges.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-6">No colleges indexed.</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/audit" className="text-[10px] font-bold text-primary hover:underline">
              VIEW ALL
            </Link>
          </div>
          <div className="divide-y divide-border">
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] text-muted-foreground truncate">
                    by @{entry.moderatorName || "system"} · {entry.targetType} ·{" "}
