            ...
            <tbody>
              {initialPosts.map((post) => (
                <tr key={post.id} className="border-b border-border hover:bg-muted/50 align-top">
                  <td className="px-5 py-4">
                    <div className="flex flex-col gap-0.5">
                      <span className="font-semibold text-foreground text-xs">
                        {post.isAnonymous
                          ? (revealed[post.id]?.displayName ?? "👻 Anonymous")
                          : (revealed[post.id]?.displayName ?? post.institution?.name?.split(",")[0])}
                      </span>
                      ...
                      {(revealed[post.id] || !post.isAnonymous) && (
                        ...
                        >
                          @{revealed[post.id]?.username ?? post.institution?.slug ?? "—"} ·{" "}
                          {revealed[post.id]?.accountStatus ?? "named"}
                        </a>
                      )}
                    </div>
                  </td>
                  <td className="px-5 py-4 text-foreground max-w-sm">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {typeof post.riskScore === "number" && post.riskScore > 0 && (
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${post.riskScore >= 70 ? "text-red-500 border-red-500/30 bg-red-500/10" : "text-orange-500 border-orange-500/30 bg-orange-500/10"}`}
                        >
                          risk {post.riskScore}
                        </span>
                      )}
                  ...
                  <td className="px-5 py-4">
                    <span
                      className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${statusBadgeTone[post.status] ?? ""}`}
                    >
                      {post.status}
                    </span>
                  </td>
