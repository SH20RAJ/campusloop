...
export function ArticleEditorClient({ initialArticle, isEditing = false }: ArticleEditorClientProps) {
  ...
  const [isDetailsOpen, setIsDetailsOpen] = useState(isEditing || Boolean(initialArticle?.coverImageUrl));
  ...
  return (
    <div className="min-h-screen pb-28 border-x border-border/30 bg-background max-w-3xl mx-auto select-none">
      {/* ─── Top Sticky Bar ─── */}
      <header className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border/30 bg-background/85 px-3 sm:px-4 backdrop-blur-xl gap-2">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          <button
            type="button"
            onClick={() => router.back()}
            className="flex size-8 sm:size-9 items-center justify-center rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-colors cursor-pointer shrink-0"
            title="Go back"
          >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 truncate">
            <span className="text-xs sm:text-sm font-black text-foreground truncate">
              {isEditing ? "Edit Article" : "Write"}
            </span>
            <span className="text-[10px] font-bold text-muted-foreground bg-muted/60 px-2 py-0.5 rounded-full border border-border/40 whitespace-nowrap shrink-0">
              {readingTime}m<span className="hidden sm:inline"> read · {wordCount}w</span>
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          ...
          <div className="flex items-center rounded-full bg-muted/60 p-0.5 border border-border/40">
            <button
              ...
              className={cn(
                "px-2 sm:px-2.5 py-1 rounded-full text-xs font-bold transition-all cursor-pointer",
                activeTab === "write"
            ...
            <button
              ...
              className={cn(
                "px-2 sm:px-2.5 py-1 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-1",
                activeTab === "preview"
                  ? "bg-foreground text-background shadow-xs font-black"
                  : "text-muted-foreground hover:text-foreground"
              )}
          ...
          <Button
            ...
            className="h-8 px-2.5 sm:px-3 rounded-full text-xs font-bold gap-1 cursor-pointer shadow-xs"
          >
          ...
          <Button
            ...
            disabled={isSubmitting}
            className="h-8 px-3 sm:px-3.5 rounded-full text-xs font-black bg-primary text-primary-foreground hover:opacity-90 shadow-md shadow-primary/20 cursor-pointer active:scale-95 gap-1.5"
          >
            <Send className="size-3.5" />
            <span className="hidden sm:inline">{isEditing ? "Update" : "Publish (+15 LP)"}</span>
          ...
          <div className="space-y-4 sm:space-y-5">
            ...
            <div>
              <input
                ...
                className="w-full bg-transparent text-xl sm:text-2xl md:text-3xl font-black text-foreground placeholder:text-muted-foreground/50 outline-none tracking-tight leading-tight"
              />
            </div>
            ...
            <div>
              <input
                ...
                className="w-full bg-transparent text-xs sm:text-sm md:text-base font-medium text-muted-foreground placeholder:text-muted-foreground/40 outline-none"
              />
            </div>

            {/* ─── Collapsible Article Details & Metadata (Cover, Category, Tags) ─── */}
            <div className="rounded-2xl border border-border/40 bg-card/60 overflow-hidden shadow-xs transition-all">
              <button
                type="button"
                onClick={() => setIsDetailsOpen(!isDetailsOpen)}
                className="w-full flex items-center justify-between px-3.5 sm:px-4 py-2.5 sm:py-3 bg-muted/20 hover:bg-muted/40 transition-colors cursor-pointer text-xs font-bold text-foreground"
              >
                <div className="flex items-center gap-2">
                  <Settings2 className="size-3.5 sm:size-4 text-primary shrink-0" />
                  <span>Article Settings (Cover, Category, Tags)</span>
                  {(coverImageUrl || tags.length > 0) && (
                    <span className="size-1.5 rounded-full bg-primary animate-pulse" />
                  )}
                </div>
                <div className="flex items-center gap-1.5 text-muted-foreground text-[11px]">
                  <span>{isDetailsOpen ? "Hide" : "Customize"}</span>
                  {isDetailsOpen ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                </div>
              </button>

              {isDetailsOpen && (
                <div className="p-3.5 sm:p-4 space-y-4 border-t border-border/30 bg-background/50">
                  {/* Cover Image Input */}
            ...
            {/* ─── Rich Markdown Formatting Toolbar (Single Row Horizontally Scrollable) ─── */}
            <div className="sticky top-14 z-30 flex items-center gap-1 rounded-2xl border border-border/40 bg-card/95 p-1.5 backdrop-blur-md shadow-sm overflow-x-auto no-scrollbar">
              <button
                type="button"
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl hover:bg-primary/10 text-primary font-black text-xs transition-colors cursor-pointer active:scale-95 disabled:opacity-60 shrink-0"
              >
                {isUploadingInline ? (
