...
export function CommunitiesClientView({
  ...
  return (
    <main className="space-y-6 max-w-4xl mx-auto pb-16">
      {/* Hero Banner Header */}
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        {filteredCommunities.map((c) => {
          ...
          return (
            <div
              key={c.id}
              className="rounded-2xl border border-border bg-card/75 p-5 shadow-sm hover:border-primary/20 hover:shadow-md hover:shadow-primary/5 transition-all duration-300 flex flex-col justify-between space-y-4 group"
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    {/* Circle Icon Badge */}
                    <div className={`h-11 w-11 rounded-xl bg-gradient-to-br ${getInitialsGradient(c.name)} flex items-center justify-center text-white text-sm font-black shadow-inner shadow-black/10 shrink-0`}>
                      {initials}
                    </div>
                    <div>
                      <Link
                        href={`/app/communities/${c.id}`}
                        className="font-bold text-foreground hover:text-primary transition-colors hover:underline text-sm leading-tight line-clamp-1"
                      >
                        {c.name}
                      </Link>
                  ...
                  <span className="text-[10px] text-primary font-bold bg-primary/10 border border-primary/20 px-2.5 py-0.5 rounded-full select-none">
        ...
        {filteredCommunities.length === 0 && (
          <div className="col-span-2 text-center py-20 border border-dashed rounded-3xl border-border bg-card/40 my-4 flex flex-col items-center justify-center p-6">
