import { NeonDbError, type NeonQueryFunction, neon } from "@neondatabase/serverless";
...
function createResilientSql(connectionString: string): NeonQueryFunction<false, false> {
  ...
  const retryingQuery = ((query: string, params?: unknown[], opts?: unknown) =>
    withRetry(() => sql.query(query, params as never, opts as never))) as unknown as NeonQueryFunction<
    false,
    false
  >;
